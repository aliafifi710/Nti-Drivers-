...
void led_on(void)
{
	//TO HAVE 0.5 sec ,NUMBER OF OV'S (FLAG) IS 30

	flag ++;
	if(flag == 30 || flag==60|| flag==90 || flag==120 || flag==150 || flag==180)
	{
		if(BIT_IS_CLEAR(PORTD->Pin.Reg,PIN7_ID))
		{
			_delay_ms(30);
			if(BIT_IS_CLEAR(PORTD->Pin.Reg,PIN7_ID))
			{
...
int main(void)
{
	...
	SET_BIT(SREG,PIN7_ID);
		...
		DIO_voidSetPinDirection(PORTD,PIN7_ID,INPUT); //push button as input
		...
		timer.VALUE=0;
		timer.PRESCALER=CLK_DIV_1024;  //WITH THIS PRESCALER TIME OF OV =16.384msec
...
//		H_LCD_void_sendString("%");
	...
	while(1)
	{
		...
		//				while(BIT_IS_CLEAR(PORTD->Pin.Reg,PIN7_ID));
